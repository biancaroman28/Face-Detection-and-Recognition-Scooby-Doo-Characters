...
MODEL_PATH = "../../models/model_daphne.joblib"
...
with open(os.path.join(TRAIN_ROOT, "daphne_annotations.txt")) as f:
    for line in f:
        ...
        if y2 - y1 >= MIN_FACE_H:
            gt_daphne[img].append((x1, y1, x2, y2))
...
print("HARD NEGATIVE MINING - DAPHNE")
...
for folder in [TARGET_CHAR] + OTHER_CHARS:
    ...
    for img_path in tqdm(img_files, desc=f"HM Daphne & {folder}"):
        ...
        for scale in SCALES:
            ...
            for y in range(0, sh - WIN_H, STEP):
                for x in range(0, sw - WIN_W, STEP):
                    ...
                    max_iou_daphne = max(
                        [iou((x1, y1, x2, y2), gt) for gt in gts_daphne],
                        default=0.0
                    )

                    if max_iou_daphne >= IOU_FP_MAX:
                        continue
...
np.save(os.path.join(OUT_DIR, "hard_neg_feats_daphne.npy"), hard_feats)

print("Hard negatives Daphne:", hard_feats.shape)
