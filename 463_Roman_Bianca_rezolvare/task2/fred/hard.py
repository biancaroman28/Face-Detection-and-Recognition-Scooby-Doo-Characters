...
TARGET_CHAR = "fred"
OTHER_CHARS = ["daphne", "shaggy", "velma"]

MODEL_PATH = "../../models/model_fred.joblib"

OUT_DIR = "../../dataset/task2/fred/"
...
gt_fred = defaultdict(list)
gt_others = defaultdict(list)

# Fred GT (TARGET)
with open(os.path.join(TRAIN_ROOT, "fred_annotations.txt")) as f:
    for line in f:
        ...
        if y2 - y1 >= MIN_FACE_H:
            gt_fred[img].append((x1, y1, x2, y2))
...
print("HARD NEGATIVE MINING - FRED")
...
for folder in [TARGET_CHAR] + OTHER_CHARS:
    ...
    for img_path in tqdm(img_files, desc=f"HM Fred & {folder}"):
        ...
        for scale in SCALES:
            ...
            for y in range(0, sh - WIN_H, STEP):
                for x in range(0, sw - WIN_W, STEP):
                    ...
                    max_iou_fred = max(
                        [iou((x1, y1, x2, y2), gt) for gt in gts_fred],
                        default=0.0
                    )

                    if max_iou_fred >= IOU_FP_MAX:
                        continue
...
np.save(os.path.join(OUT_DIR, "hard_neg_feats_fred.npy"), hard_feats)

print("Hard negatives Fred:", hard_feats.shape)
