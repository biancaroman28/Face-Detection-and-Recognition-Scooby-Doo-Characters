...
TARGET_CHAR = "shaggy"
OTHER_CHARS = ["daphne", "fred", "velma"]

MODEL_PATH = "../../models/model_shaggy.joblib"

OUT_DIR = "../../dataset/task2/shaggy/"
...
gt_shaggy = defaultdict(list)
gt_others = defaultdict(list)


with open(os.path.join(TRAIN_ROOT, "shaggy_annotations.txt")) as f:
    for line in f:
        ...
        if y2 - y1 >= MIN_FACE_H:
            gt_shaggy[img].append((x1, y1, x2, y2))
...
print("HARD NEGATIVE MINING - SHAGGY")
...
for folder in [TARGET_CHAR] + OTHER_CHARS:
    ...
    for img_path in tqdm(img_files, desc=f"HM Shaggy & {folder}"):
        ...
        gts_shaggy = gt_shaggy.get(img_name, [])
        ...
        for scale in SCALES:
            ...
            for y in range(0, sh - WIN_H, STEP):
                for x in range(0, sw - WIN_W, STEP):
                    ...
                    max_iou_shaggy = max(
                        [iou((x1, y1, x2, y2), gt) for gt in gts_shaggy],
                        default=0.0
                    )

                    if max_iou_shaggy >= IOU_FP_MAX:
                        continue
...
np.save(os.path.join(OUT_DIR, "hard_neg_feats_shaggy.npy"), hard_feats)


print("Hard negatives Shaggy:", hard_feats.shape)
