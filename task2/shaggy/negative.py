...
with open(os.path.join(TRAIN_ROOT, "shaggy_annotations.txt")) as f:
    for line in f:
        ...
        if y2 - y1 >= MIN_FACE_H:
            gt_target.setdefault(img, []).append((x1, y1, x2, y2))
...
for sub in [TARGET_CHAR] + OTHER_CHARS:
    ...
    for img_path in img_files:
        ...
        gts_target = gt_target.get(img_name, [])
        ...
        while (got_near < target_near or got_random < target_random) \
            ...
            if any(compute_iou(cand, gt) >= IOU_NEAR_MIN for gt in gts_target):
                continue
...
print("Negative Shaggy:", negatives.shape)
