...
CHAR_FOLDERS = ["daphne", "fred", "shaggy", "velma"]
...
OUT_DIR = Path("../dataset/task1/")
OUT_DIR.mkdir(parents=True, exist_ok=True)

# raportul e mai apropiat de forma fetei, care e mai inalta decat lata 
# ceea ce reiese din bbox-urile de antrenare
WINDOW_W = 64
WINDOW_H = 80

# pentru fiecare fata se genereaza inca 4 variante
...
for folder in CHAR_FOLDERS:
    ...
    ann_path = BASE_DIR / ANN_FILES[folder]
    ...
    with open(ann_path, "r", encoding="utf-8") as f:
        for line in f:
            ...
            img_name, x1, y1, x2, y2, _ = parts
            x1, y1, x2, y2 = map(int, [x1, y1, x2, y2])
            
            # fete prea mici sunt ignorate
            bh = y2 - y1
            bw = x2 - x1
            if bh < MIN_FACE_H:
                continue
            ...
            # luam context in jurul fetei
            # padding proportional cu dimensiunea fetei
            # pentru fete mai mici, mai mult context
            if bh < 40:
                PAD_FRAC = 0.18
            elif bh < 80:
                PAD_FRAC = 0.14
...
# np.save(OUT_DIR / "pos_chars.npy", np.array(pos_chars))
# np.save(OUT_DIR / "pos_heights.npy", np.array(pos_heights))

print("Pozitive:", len(positives))
